...
![Confidence Dilemma](/assets/img/Confidence Dilemma.png)
_The gap between offline accuracy and live prediction confidence highlighted a fundamental issue with the system's reliability._
...
The calibration pipeline was integrated into the existing `merge_and_retrain_1400.py` script with 5-fold internal cross-validation, running the same 10 random seeds as the uncalibrated baseline for direct comparison. The results were immediately telling: uncalibrated mean confidence was already 98.0% and the calibrated version actually dropped slightly. Calibration had no room to improve because the offline probabilities were already well-calibrated. The low live confidence scores were not a calibration problem.
